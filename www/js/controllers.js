  ...
  $scope.validatePasswords = function(backup) {
    ...
    $timeout(function() {
      ...
      .then(function(master_key) {
         BitShares.encryptString(master_key.key, backupForm.backupPassword.value).then(
  ...
  $scope.restoreWallet = function(restore){
    ...
    $ionicPopup.prompt({
      ...
      $timeout(function() {
        ...
        BitShares.decryptString(master_key.key, password).then(
          function(decryptedData){

            master_key.key = decryptedData;
            
            BitShares.isValidKey(master_key.key).then(function(valid){
            ...
            alertPopup.close();
            $ionicPopup.alert({
              title    : T.i('err.unable_decrypt_wallet'),
              template : T.i('err.unable_decrypt_wallet_desc')
            });
            return;
        });
      }, 500);  
  ...
  $scope.newAddr = function(){
     ...
     var confirmPopup = $ionicPopup.confirm({
      ...
      MasterKey.get().then(function(master_key) {
        ...
        BitShares.derivePrivate(master_key.key, master_key.deriv).then(function(new_priv){
          BitShares.extractDataFromKey(new_priv).then(function(keyData){
     ...
     buttonClicked: function(index) {
      ...
      if(index==0)
      {
        Asset.setDefault(asset.id).then(function() {
          $location.path('/home');
          $rootScope.assetChanged(asset.asset_id);
        });
      }
      // Hide asset
      else if(index==1)
...
.controller('ReceiveQrcodeCtrl', function($scope, T, $cordovaClipboard, $cordovaSocialSharing, Address, $stateParams, $http, $ionicNavBarDelegate, $location, $ionicLoading, $timeout, $ionicModal, $ionicPopup) {
  ...
  $scope.request    = 'bts:'+$scope.address+'/transfer/amount/'+$scope.amount+'/asset/'+$scope.asset.symbol; 
...
.controller('ImportPrivCtrl', function($scope, T, $q, Address, $http, $ionicLoading, $ionicNavBarDelegate, $ionicModal, $ionicPopup, $location, $timeout, $rootScope, $stateParams) {
  ...
  $scope.sweepBalance = function(){
    ...
    Address.getDefault().then(function(my_default_address) {
      ...
      .success(function(r) {
        ...
        BitShares.compactSignatureForHash(r.to_sign, $scope.imported_pk.priv_key).then(
          function(compactSignatureForHash){
        
            console.log(compactSignatureForHash);

            r.tx.signatures.push(compactSignatureForHash);
  ...
  $scope.onInit = function(){
    ...
    // = bitcoin.bts.wif_to_address($scope.imported_pk.priv_key);

    BitShares.btsWifToAddress($scope.imported_pk.priv_key).then(
      function(addy){
        $scope.imported_pk.address = addy;
        $timeout( function() { 
...
.controller('SendCtrl', function($scope, $q, T, AddressBook, Scanner, Address, $http, $ionicLoading, $ionicNavBarDelegate, $ionicModal, $ionicPopup, $location, $timeout, $rootScope, $stateParams) {
  ...
  if (!angular.isUndefined($stateParams.asset_id))
  {
    ...
    $scope.asset = $rootScope.assets[$stateParams.asset_id];
  }
  ...
  $scope.transaction = {message:'send.generating_transaction', amount:amount, address:address, asset_id:$scope.asset_id};
  sendForm.transactionAmount.value = amount;
  sendForm.transactionAddress.value = address;
  sendForm.transactionAssetId.value = $scope.asset_id;
  
  
  $scope.loadDefaultAsset = function(){
    if($scope.asset_id!==undefined & $scope.asset_id!=0)
      return;
    
    $scope.asset_id                   = $rootScope.asset_id;
    $scope.transaction.asset_id       = $scope.asset_id;
    sendForm.transactionAssetId.value = $scope.asset_id;
    
    try {
      $scope.asset = $rootScope.assets[$scope.asset_id];
    } 
    catch(err) {
    }
  }
    ...
    .then(function(result) {
      if( !result.cancelled ) {
        ...
        if(result.pubkey !== undefined) {
          ...
          BitShares.btsPubToAddress(result.pubkey).then(
          function(addy){
        ...
        if(result.asset_id !== undefined)
        {
          ...
          var the_asset = $rootScope.getValidAssetId(result.asset_id);
          if(!the_asset)
          {  
            $scope.transaction.asset_id = -1;
            sendForm.transactionAssetId.value = -1;
            $scope.asset = [];
            $timeout(
          ...
          $scope.transaction.asset_id = the_asset;
          sendForm.transactionAssetId.value = $scope.transaction.asset_id;
          $scope.asset = $rootScope.assets[$scope.transaction.asset_id];
        }
      }
  ...
  $scope.validateSend = function(transaction) {
    ...
    var my_asset_id = $scope.transaction.asset_id;
    if(!$rootScope.isValidAsset(my_asset_id))
    {
      $ionicPopup.alert({
         title    : T.i('err.invalid_asset') + ' <i class="fa fa-warning float_right"></i>',
         template : T.i('err.invalid_asset_desc'),
         okType   : 'button-assertive', 
       });
       return;
    }
    ...
    BitShares.btsIsValidAddress(sendForm.transactionAddress.value).then(
      function(is_valid){
        // var my_asset = [];
        // angular.copy($scope.asset, my_asset);
        // if(my_asset === undefined || my_asset.length==0)
        //    my_asset = $rootScope.assets[$scope.transaction.asset_id] ;
        ...
        confirmPopup.then(function(res) {
          if(res) {
            ...
            var tx_req = {
              "asset" : $scope.transaction.asset_id, // ? sendForm.transactionAssetId.value ?
              "fee"   : 250,
              "from"  : from,
              "to"    : [{
            ...
            .success(function(r) {
              ...
              angular.forEach(r.required_signatures, function(req_addy) {
                var p = Address.by_address(req_addy).then(function(addy) {
                  ...
                  var p2 = BitShares.compactSignatureForHash(r.to_sign, addy.privkey).then(
                    function(compactSignatureForHash){
                      r.tx.signatures.push(compactSignatureForHash); 
                    });
                  prom.push(p2);  
  ...
  $scope.scanQR = function() {
    ...
    .then(function(result) {
      if( !result.cancelled ) {
        ...
        //Pubkey scanned
        if(result.pubkey !== undefined) {
          var p = BitShares.btsPubToAddress(result.pubkey).then(
          function(addy){
            result.address = addy;
